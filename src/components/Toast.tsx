...
export function ToastViewport({ messages }: { messages: ToastMessage[] }) {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 grid w-[calc(100%-2rem)] max-w-sm gap-3" aria-live="polite">
      {messages.map((toast) => {
        ...
        return (
          <div key={toast.id} className={`flex gap-3 rounded-lg border bg-surface p-4 shadow-overlay ${toneClass}`}>
